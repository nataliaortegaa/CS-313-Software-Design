...
class ImageGraph:
    ...
    def print_adjacency_matrix(self):
        ...
        for node in self.nodes:
            for edge_index in node.edges:
                adjacency_matrix[node.index][edge_index] = 1
    ...
    def bfs(self, start_index, color):
        ...
        start_color = start_node.color  # Store the start node color for comparison
        queue.enqueue(start_node)

        while not queue.is_empty():
            current_node = queue.dequeue()

            # Check if the current node is of the same color as the start node
            if current_node.color == start_color:
